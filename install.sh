...
echo "For Mac and Windows, refer to the official guide https://github.com/anggipradana/ReNgGinaNg"
echo "#########################################################################"

echo " "
tput setaf 4;
echo "Installing ReNgGinaNg and its dependencies"

echo " "
if [ "$EUID" -ne 0 ]
  then
  tput setaf 1; echo "Error installing ReNgGinaNg, Please run this script as root!"
...
tput setaf 4;
echo "#########################################################################"
echo "Installing ReNgGinaNg"
echo "#########################################################################"
make certs && make build && make up && tput setaf 2 && echo "ReNgGinaNg is installed!!!" && failed=0 || failed=1
...
if [ "${failed}" -eq 0 ]; then
  ...
  tput setaf 2 && printf "\n%s\n" "Thank you for installing ReNgGinaNg, happy recon!!"
  echo "In case you have unapplied migrations (see above in red), run 'make migrate'"
else
  tput setaf 1 && printf "\n%s\n" "ReNgGinaNg installation failed!!"
fi
