...
def api_vault(request, slug):
    ...
    if request.method == "POST":
        ...
        key_wpscan = request.POST.get('key_wpscan')
        key_otx = request.POST.get('key_otx')
        key_leakcheck = request.POST.get('key_leakcheck')
        ...
        if key_wpscan:
            wpscan_api_key = WPScanAPIKey.objects.first()
            if wpscan_api_key:
                wpscan_api_key.key = key_wpscan
                wpscan_api_key.save()
            else:
                WPScanAPIKey.objects.create(key=key_wpscan)

        if key_otx:
            otx_api_key = OTXAlienVaultAPIKey.objects.first()
            if otx_api_key:
                otx_api_key.key = key_otx
                otx_api_key.save()
            else:
                OTXAlienVaultAPIKey.objects.create(key=key_otx)

        if key_leakcheck:
            leakcheck_api_key = LeakCheckAPIKey.objects.first()
            if leakcheck_api_key:
                leakcheck_api_key.key = key_leakcheck
                leakcheck_api_key.save()
            else:
                LeakCheckAPIKey.objects.create(key=key_leakcheck)

    # Only send booleans to the template — never expose actual keys
    context['openai_key'] = OpenAiAPIKey.objects.exists()
    context['netlas_key'] = NetlasAPIKey.objects.exists()
    context['chaos_key'] = ChaosAPIKey.objects.exists()
    h1_key = HackerOneAPIKey.objects.first()
    context['hackerone_key'] = bool(h1_key)
    context['hackerone_username'] = h1_key.username if h1_key else ''
    context['wpscan_key'] = WPScanAPIKey.objects.exists()
    context['otx_key'] = OTXAlienVaultAPIKey.objects.exists()
    context['leakcheck_key'] = LeakCheckAPIKey.objects.exists()
