...
from ReNgGinaNg.common_func import (fmt_traceback, get_output_file_name,
...
class RengineTask(Task):
	...
	def __call__(self, *args, **kwargs):
		...
		self.results_dir = ctx.get('results_dir', RENGGINANG_RESULTS)
		...
		if RENGGINANG_CACHE_ENABLED:
			...
			if result and result != b'null':
				...
				if RENGGINANG_RECORD_ENABLED and self.track:
		...
		if RENGGINANG_CACHE_ENABLED and self.status == SUCCESS_TASK and result:
	...
	def write_results(self):
		try:
			...
			logger.error(f'Failed to write results for {self.task_name}: {e}')
	...
	def notify(self, name=None, severity=None, fields={}, add_meta_info=True):
		...
		from ReNgGinaNg.tasks import send_task_notif
