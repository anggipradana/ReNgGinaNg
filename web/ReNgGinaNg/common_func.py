...
from ReNgGinaNg.common_serializers import *
...
from ReNgGinaNg.utilities import is_valid_url
...
def get_http_urls(
	...
	if ignore_files: # ignore all files
		extensions_path = f'{RENGGINANG_HOME}/fixtures/extensions.txt'
...
def send_google_chat_message(message):
	...
	do_send = (
		notif and
		notif.send_to_google_chat and
		notif.google_chat_hook_url)
	if not do_send:
		return
	hook_url = notif.google_chat_hook_url
	requests.post(url=hook_url, data=json.dumps(payload), headers=headers)
...
def get_task_cache_key(func_name, *args, **kwargs):
	...
	kwargs_str = '_'.join([f'{k}={v}' for k, v in kwargs.items() if k not in RENGGINANG_TASK_IGNORE_CACHE_KWARGS])
...
def get_domain_historical_ip_address(domain):
	...
	for row in table or []:
		try:
			...
			last_seen = row.findAll('td')[3].getText()
			if ip == 'IP Address':
				continue
			ips.append(
				{
...
def get_otx_alienvault_key():
	otx_key = OTXAlienVaultAPIKey.objects.all()
	return otx_key[0] if otx_key else None


def get_leakcheck_key():
	leakcheck_key = LeakCheckAPIKey.objects.all()
	return leakcheck_key[0] if leakcheck_key else None
