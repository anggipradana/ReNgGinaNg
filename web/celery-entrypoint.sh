...
if [ -f /usr/src/github/EyeWitness/setup/requirements.txt ]; then
  python3 -m pip install -r /usr/src/github/EyeWitness/setup/requirements.txt
fi

# clone theHarvester (pinned to 4.6.0 for Python 3.10 compatibility)
if [ ! -d "/usr/src/github/theHarvester" ]
then
  echo "Cloning theHarvester"
  git clone --branch 4.6.0 --depth 1 https://github.com/laramies/theHarvester /usr/src/github/theHarvester
fi
...
elif [ -f /usr/src/github/theHarvester/pyproject.toml ]; then
  python3 -m pip install /usr/src/github/theHarvester
fi
...
then
  ...
  echo "Usage in ReNgGinaNg, set vulscan/vulscan.nse in nmap_script scanEngine port_scan config parameter"
fi
...
# Reinstall pinned tenacity after external tool installs (they may downgrade it)
python3 -m pip install tenacity==8.2.2
